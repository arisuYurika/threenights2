...
Ycx.urlText = String(Ycx.parameters['urlText'] || 'https://gitee.com/yuban10043/three-nights-talk-chapter2-version-check/blob/master/version.txt');
Ycx.showInTitle = String(Ycx.parameters['showInTitle'] || 'true');
Ycx.showInTitleText = String(Ycx.parameters['showInTitleText'] || '版本检测');
Ycx.urlOpen = String(Ycx.parameters['urlOpen'] || 'https://share.weiyun.com/Py5vWAYF');
...
Game_Interpreter.prototype.checkVersion = function(args) {
    ...
    $(document).load(url, function(responseText, textStatus) {
        if (Ycx.nowVersion == responseText) {
            alert(`当前版本 Ver${Ycx.nowVersion} 已经是最新版本了。`);
        } else {
            if (confirm(`存在最新版本: Ver${responseText}是否前往下载页面？`)) {
